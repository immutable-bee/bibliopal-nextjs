...
function ButtonComponent({ children, loading = false, onClick, type = 'button', icon, id, className = '', rounded = false, color = 'yellow', full = false, align = 'center', padding = 'normal', border = true, disabled = false, onlyIcon = false }) {
  const colorMap = {
  ...
  return (
    ...
    >
      {loading ?
        ...
        >
          <svg class={`${loadingMap[padding]} animate-spin`} viewBox="3 3 18 18">
            <path
              class="fill-sky-100"
